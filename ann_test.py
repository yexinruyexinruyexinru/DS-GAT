...
presim_len=4
sim_len=8
model = replace_activation_by_floor(model, 16).to(device)
model.eval()
with torch.no_grad():
    correct=0
    logits=[]
    labels=[]
    for nodes in test_loader:
        logits.append(model(nodes))
        labels.append(y[nodes])
    logits = torch.cat(logits, dim=0).cpu()
    labels = torch.cat(labels, dim=0).cpu()
    ...
    metric_macro = metrics.f1_score(labels, logits, average='macro')
    metric_micro = metrics.f1_score(labels, logits, average='micro')
    print("QCFS: macro为： %.04f; micro为 %.04f" % ( metric_macro, metric_micro))
    # print("转换为QCFS的准确率为:%.04f"%(acc))
...
with torch.no_grad():
    ...
    metric_macro = metrics.f1_score(labels, logits, average='macro')
    metric_micro = metrics.f1_score(labels, logits, average='micro')
    print("SNN: macro为： %.04f; micro为 %.04f" % (metric_macro, metric_micro))
