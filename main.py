...
# 添加注意力机制导入的模块
from spikenet.layers import GAT

class SpikeNet(nn.Module):
    def __init__(self, in_features, out_features, hids=[32], alpha=1.0, p=0.5,
                 dropout=0.7, bias=True, aggr='mean', sampler='sage',
                 surrogate='triangle', sizes=[5, 2], concat=False, act='LIF',atten=0):

        super().__init__()

        tau = 1.0
        # sampler得到的是采样得到的矩阵,具体的形式是什么样的？
        if sampler == 'rw':
        ...
        for hid in hids:
            aggregators.append(GAT(in_features, hid,alpha,
                                              concat=concat))
            if act == "IF":
    ...
    def encode(self, nodes):
        ...
        for time_step in range(len(data)):
            ...
            for size in sizes:
                ...
                x[nbr_1]*=(1-(self.k/10)*(size_1/size))
                x[nbr_2]*=(1+(self.k/10)*(size_2/size))
...
parser.add_argument('--atten', type=int, default=0,
                    help='default attention factor. (default: 0)')
...
model = SpikeNet(data.num_features, data.num_classes, alpha=args.alpha,
                 ...
                 hids=args.hids, act=args.neuron, atten=args.atten, bias=True).to(device)
