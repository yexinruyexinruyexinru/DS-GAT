...
class Net(torch.nn.Module):
    ...
    def forward(self,data):
        for i in range(1):
            if i==0:
                x=self.snn_list[i](data.x[len(data.x)-i-1],data.edges[len(data.x)-i-1],data.edges_weight[len(data.x)-i-1])
            else:
                x=x+(0.8-0.03*i)*self.snn_list[i](data.x[len(data.x)-i-1],data.edges[len(data.x)-i-1])
        # x=self.conv1(x,edge_index,edges_weight)
        x = self.relu1(x)
